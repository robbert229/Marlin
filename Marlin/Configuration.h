...
#define STRING_CONFIG_H_AUTHOR "(johnrowl, Ender-3)" // Who made the changes.
 ...
 * port pins. Note: The first serial port (-1 or 0) will always be used by the
 * Arduino bootloader.
 *
 * :[-1, 0, 1, 2, 3, 4, 5, 6, 7]
 */
 ...
 * host. This allows the connection of wireless adapters (for instance) to
 * non-default port pins. Serial port -1 is the USB emulated serial port, if
 * available.
 *
 * :[-1, 0, 1, 2, 3, 4, 5, 6, 7]
 */
...
#define MPE_FAST_SPEED \
  9000 // (mm/m) Speed for travel before last distance point
#define MPE_SLOW_SPEED \
  4500                         // (mm/m) Speed for last distance travel to park and couple
...
#define SWITCHING_TOOLHEAD_RETRACT_FEEDRATE \
  2400 // (mm/m) Extruder retract feedrate
#endif
...
// M80 #define PSU_POWERUP_DELAY 100   // (ms) Delay for the PSU to warm up to
// full power
 ...
 * (Used in ParCan & J-Head) (4.7k pullup) 501 : 100K Zonestar (Tronxy X3A)
 * Thermistor 512 : 100k RPW-Ultra hotend thermistor (4.7k pullup) 6 : 100k
 ...
 * thermistor (4.7k pullup) 12 : 100k 0603 SMD Vishay NTCS0603E3104FXT (4.7k
 * pullup) (calibrated for Makibox hot bed) 13 : 100k Hisens 3950  1% up to
 * 300°C for hotend "Simple ONE " & "Hotend "All In ONE" 15 : 100k thermistor
 * calibration for JGAurora A5 hotend 18 : ATC Semitec 204GT-2 (4.7k pullup)
 * Dagoma.Fr - MKS_Base_DKU001327 20 : Pt100 with circuit in the Ultimainboard
 * V2.x 201 : Pt100 with circuit in Overlord, similar to Ultimainboard V2.x 60 :
 * 100k Maker's Tool Works Kapton Bed Thermistor beta=3950 61 : 100k Formbot /
 * Vivedino 3950 350C thermistor 4.7k pullup 66 : 4.7M High Temperature
 * thermistor from Dyze Design 67 : 450C thermistor from SliceEngineering 70 :
 * the 100K thermistor found in the bq Hephestos 2 75 : 100k Generic Silicon
 * Heat Pad with NTC 100K MGB18-104F39050L32 thermistor 99 : 100k thermistor
 * with a 10K pull-up resistor (found on some Wanhao i3 machines)
 *
 ...
 * :['A4988', 'A5984', 'DRV8825', 'LV8729', 'L6470', 'TB6560', 'TB6600',
 * 'TMC2100', 'TMC2130', 'TMC2130_STANDALONE', 'TMC2160', 'TMC2160_STANDALONE',
 * 'TMC2208', 'TMC2208_STANDALONE', 'TMC2209', 'TMC2209_STANDALONE', 'TMC26X',
 ...
 *
 */
// #define Z_MIN_PROBE_PIN P1_24 // 32 // Pin 32 is the RAMPS default
...
#define NUM_RUNOUT_SENSORS \
  1 // Number of sensors, up to one per extruder. Define a FIL_RUNOUT#_PIN for \
      // each.
#define FIL_RUNOUT_INVERTING \
  false                   // Set to true to invert the logic of the sensor.
...
#define MESH_TEST_LAYER_HEIGHT \
  0.2 // (mm) Default layer height for the G26 Mesh Validation Tool.
#define MESH_TEST_HOTEND_TEMP \
  205 // (°C) Default nozzle temperature for the G26 Mesh Validation Tool.
#define MESH_TEST_BED_TEMP \
  60 // (°C) Default bed temperature for the G26 Mesh Validation Tool.
#define G26_XY_FEEDRATE \
  20 // (mm/s) Feedrate for XY Moves for the G26 Mesh Validation Tool.
#endif
...
#define LEVEL_CORNERS_Z_HOP \
  4.0                            // (mm) Move nozzle up before moving between corners
...
#define Z_SAFE_HOMING_X_POINT \
  ((X_BED_SIZE) / 2) // X point for Z homing when homing all axes (G28).
#define Z_SAFE_HOMING_Y_POINT \
  ((Y_BED_SIZE) / 2) // Y point for Z homing when homing all axes (G28).
#endif
 ...
 * 'jp_kana':'Japanese', 'ko_KR':'Korean (South Korea)', 'nl':'Dutch',
 * 'pl':'Polish', 'pt':'Portuguese', 'pt_br':'Portuguese (Brazilian)',
 * 'ru':'Russian', 'sk':'Slovak', 'tr':'Turkish', 'uk':'Ukrainian',
 * 'vi':'Vietnamese', 'zh_CN':'Chinese (Simplified)', 'zh_TW':'Chinese
 * (Traditional)', 'test':'TEST' }
 */
...
//(always on) #define FYSETC_MINI_12864_2_0  // Type A/B. Discreet RGB Backlight
...
#define NEOPIXEL_PIXELS \
  30                            // Number of LEDs in the strip, larger of 2 strips if 2 neopixel strips are \
                                // used
